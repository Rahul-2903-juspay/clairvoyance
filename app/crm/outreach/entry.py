...
async def _wake_on_reply(
    ...
    if current is None or listens(current):
        return
    # A call report answers its own square or nothing. Refreshed here it
    # would land at the TOP level — the next call's payload would speak a
    # late report's outcome, the leak _reply_patch closes on the reply path —
    # and it would re-arm a parked run on a report that decides nothing.
